...
# setup for AWS cli
aws sts get-caller-identity --query Arn | grep eks-c9-admin > /dev/null
if [ $? -eq 0 ]; then
...
if [ ! `which kubectl 2> /dev/null` ]; then
  ...
  curl --silent -LO https://storage.googleapis.com/kubernetes-release/release/v1.23.10/bin/linux/amd64/kubectl  > /dev/null
  chmod +x ./kubectl
